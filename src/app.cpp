...
vec3 vmPresetOffset(int p) { return p == 1 ? vec3(-1.2f, -0.2f, -1.2f) : p == 2 ? vec3(0.6f, -1.0f, -0.8f) : vec3(0); }
...
int App::run(int argc, char** argv) {
    ...
    while (!win.shouldClose() && !quit) {
        ...
        theme.dt = dt;
        if (state == ST_LOBBY) frameLobby(dt);
        else frameGame(dt);
...
bool App::boot() {
    ...
    {
        WorldTextureSet ts;
        generateWorldTextures(ts, 512);
        std::vector<const void*> al, nl;
        for (int i = 0; i < MAT_COUNT; i++) {
            al.push_back(ts.albedo[i].data());
            nl.push_back(ts.normal[i].data());
        }
        albedoArr = makeTextureArray(512, 512, MAT_COUNT, GL_SRGB8_ALPHA8, GL_RGBA, GL_UNSIGNED_BYTE, al, 16);
        normalArr = makeTextureArray(512, 512, MAT_COUNT, GL_RGBA8, GL_RGBA, GL_UNSIGNED_BYTE, nl, 16);
        renderer.setWorldTextures(albedoArr, normalArr);
    }
...
void App::applyVideoSettings() {
    ...
    rs.shadowSize = settings.shadows == 0 ? 0 : settings.shadows == 1 ? 2048 : 4096;
...
static void uiInput(UI& ui, const Window& win, bool automation) {
    ...
    ui.mouse = automation ? vec2(-1000, -1000) : in.mousePos;
        ...
        switch (e.type) {
            case EV_SHOT: {
                ...
                if (loc) {
                    ...
                    shake = std::max(shake, d.cls == WC_SNIPER ? 0.6f : 0.12f);
                } else {
...
void App::frameGame(float dt) {
    ...
    if (me.alive) {
        ...
        cp = clampf(viewPitch + me.recoil.y + sy, -89, 89);
        cy = viewYaw - me.recoil.x + sx;
        cam.setAngles(eye, cp, cy);
        if (shot.thirdPerson) {
    ...
    if (firstPerson && me.active != W_NONE) {
        ...
        vp.userOffset = vmPresetOffset(settings.vmPreset);
        const WeaponModel& wm = weapons[me.active];
        animateViewmodel(wm, vp, vmState, vmPose);
        Xform eyeX(cam.pos, quatFromEuler(cp, cy, 0));
        buildArmBones(arms[me.team], wm, vmPose, eyeX, armBones);
        vmWeaponWorld = eyeX * vmPose.weapon;
        vmMuzzleWorld = apply(vmWeaponWorld, wm.muzzle);
        vec3 au, ad;
        map.sampleAmbient(cam.pos, au, ad);
        ModelDraw a;
        a.mesh = &arms[me.team].mesh;
        a.bones = armBones;
        a.boneCount = AB_PER_ARM * 2;
        a.patternA = arms[me.team].patternA;
        a.patternB = arms[me.team].patternB;
        a.ambUp = au;
        a.ambDown = ad;
        a.castShadow = false;
        vmDraws.push_back(a);
